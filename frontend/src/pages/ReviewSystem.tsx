...
export default function ReviewSystem({ user }: { user: User }) {
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div><h1 className="text-xl font-bold font-heading">Review & Shortlisting</h1>
            <p className="text-sm text-slate-500">Select form → view submissions → shortlist → assign reviewers → next levels</p></div>
        </div>

        {/* Form selector */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          <label className="text-xs font-semibold text-slate-500 mb-2 block">Select Form to Review</label>
          <div className="flex gap-3 items-end">
            <select value={selectedFormId} onChange={e => { const id = e.target.value; if (id) loadFormData(id); else { setSelectedFormId(''); setShortlistData(null); } }}
              className="flex-1 px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none focus:border-primary">
        ...
        {selectedFormId && shortlistData && !loadingSubs && (
          <>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              {formLevels.length === 0 ? (
                <div className="text-center py-6 text-slate-500"><p className="text-sm">No review levels yet. Create a shortlist to start the pipeline.</p></div>
              ) : (
                <div className="flex items-center gap-2 overflow-x-auto pb-2">
                  <div className="flex-shrink-0 p-3 rounded-xl bg-blue-50 border border-blue-200 text-center min-w-[120px]">
                  ...
                  {formLevels.map((l: any, i: number) => {
                    ...
                    return (<React.Fragment key={l.id}>
                      ...
                      <div className="flex-shrink-0 p-3 rounded-xl bg-slate-100 border border-slate-200 text-center min-w-[140px]">
                        ...
                        <p className="text-[9px] text-slate-500">{l.scoring_type?.replace('_', ' ')} · {l.blind_review ? 'Blind' : 'Open'}</p>
                      </div>
                    </React.Fragment>);
                  })}
                </div>
        ...
        <Modal open={showShortlist} onClose={() => setShowShortlist(false)} title={`Create Shortlist — Level ${levelForm.level_number}`} size="xl">
          <div className="space-y-5">
            <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
              <p className="text-xs text-blue-700 font-medium">This will filter submissions and assign them to reviewers for Level {levelForm.level_number}.</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Filter Type</label>
                  <select value={shortlistFilter.filter_type} onChange={e => setShortlistFilter(p => ({ ...p, filter_type: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none">
                ...
                {(shortlistFilter.filter_type === 'form_score_gte' || shortlistFilter.filter_type === 'review_avg_gte') && (
                  <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Minimum Score</label>
                    <input type="number" value={shortlistFilter.filter_value} onChange={e => setShortlistFilter(p => ({ ...p, filter_value: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none" placeholder="e.g. 80" /></div>
                )}
                {shortlistFilter.filter_type === 'review_avg_gte' && (
                  <div><label className="text-xs font-semibold text-slate-500 mb-1 block">From Level</label>
                    <select value={shortlistFilter.source_level_id} onChange={e => setShortlistFilter(p => ({ ...p, source_level_id: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none">
                ...
                {shortlistFilter.filter_type === 'field_value' && (() => {
                  ...
                  return (<>
                    <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Field to Filter</label>
                      <select value={shortlistFilter.field_id} onChange={e => setShortlistFilter(p => ({ ...p, field_id: e.target.value, field_value: '' }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none">
                        <option value="">Select field</option>
                        {flat.map((f: any) => <option key={f.id} value={f.id}>{f.label} ({f.type})</option>)}
                      </select></div>
                    <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Equals Value</label>
                      {selectedField?.options ? (
                        <select value={shortlistFilter.field_value} onChange={e => setShortlistFilter(p => ({ ...p, field_value: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none">
                          <option value="">Select value</option>
                          {selectedField.options.map((o: string) => <option key={o} value={o}>{o}</option>)}
                        </select>
                      ) : (
                        <input value={shortlistFilter.field_value} onChange={e => setShortlistFilter(p => ({ ...p, field_value: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none" placeholder="Enter value to match" />
                      )}</div>
                  </>);
                })()}
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Level Name</label>
                  <input value={levelForm.name} onChange={e => setLevelForm(p => ({ ...p, name: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none" placeholder='e.g. "Level 1 - High Scorers"' /></div>
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Scoring Type</label>
                ...
                <div><label className="text-xs font-semibold text-slate-500 mb-1 block">Grade Scale</label>
                  <input value={levelForm.grade_scale} onChange={e => setLevelForm(p => ({ ...p, grade_scale: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none" placeholder="A,B,C,D" /></div>
                <div className="flex items-end pb-1"><label className="flex items-center gap-2 text-xs font-semibold text-slate-500"><input type="checkbox" checked={levelForm.blind_review} onChange={e => setLevelForm(p => ({ ...p, blind_review: e.target.checked }))} className="rounded accent-primary" /> Blind Review (hide submitter name)</label></div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {reviewers.map(r => (
                  <label key={r.id} className={`flex items-center gap-2 p-3 rounded-xl border cursor-pointer transition-all min-h-[44px] ${levelForm.reviewer_ids.includes(r.id) ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-slate-200 hover:border-muted'}`}>
                    ...
                    <div><p className="text-xs font-semibold">{r.name}</p><p className="text-[9px] text-slate-500">{r.email}</p></div>
                  </label>
                ))}
              </div>
            </div>
            ...
            {shortlistResult && (
              <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200">
                <p className="text-sm text-emerald-700 font-bold flex items-center gap-2"><CheckCircle size={16} /> Shortlist created!</p>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowShortlist(false)} className="px-4 py-2 text-sm rounded-xl border border-slate-200 hover:bg-slate-100">Close</button>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {displayed.length === 0 ? <div className="text-center py-12 text-slate-500 text-sm">No {reviewTab} reviews</div> :
          displayed.map(r => (
            <div key={r.id} onClick={() => r.status === 'pending' ? openReview(r) : openProfile(r.submission_id)}
              className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500">Level {r.level} · {r.reviewer_name}</p>
              </div>
          ...
          <div className="space-y-5">
            {selectedSub && Object.keys(subResponses).length > 0 && (
              <div><h4 className="text-sm font-bold mb-2">Responses</h4>
                <div className="bg-slate-100 rounded-xl p-4 space-y-2 max-h-60 overflow-y-auto">
                  {Object.entries(subResponses).map(([k, v]) => (
                    <div key={k} className="flex flex-col sm:flex-row gap-1 py-1 border-b border-slate-200 last:border-0">
                      <span className="text-xs font-semibold text-slate-500 min-w-[140px]">{k}:</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Score (0-100)</label>
                <input type="number" min={0} max={100} value={overallScore} onChange={e => setOverallScore(parseInt(e.target.value) || 0)} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none focus:border-primary" /></div>
            ...
            <div><label className="text-xs font-semibold text-slate-500 mb-1.5 block">Comments</label>
              <textarea value={reviewComment} onChange={e => setReviewComment(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-slate-100 text-sm outline-none h-24 resize-none" /></div>
            <div className="flex gap-3">
              <button onClick={saveDraft} className="px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-sm font-semibold hover:bg-white flex items-center gap-2 min-h-[44px]"><Save size={14} /> Draft</button>
          ...
          return (
            <div className="space-y-5">
              <div className="bg-slate-100 rounded-xl p-4">
                ...
                <p className="text-xs text-slate-500">{sub.user_email} · {sub.form_title}</p>
                <div className="flex items-center gap-2 mt-2"><StatusBadge status={sub.status} /><span className="text-xs text-slate-500">Level {profileData.highest_level}/{profileData.total_levels}</span></div>
              </div>
              {profileData.levels.map((lvl: any) => (
                <div key={lvl.level_id} className="p-4 rounded-xl border border-slate-200">
                  <div className="flex items-center justify-between mb-2">
                  ...
                  {lvl.scores.map((s: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 p-2 bg-slate-100 rounded-lg mt-1">
                      <span className="text-xs font-bold">Score: {s.overall_score}</span>
                      {s.grade && <span className="text-[10px] bg-white border border-slate-200 px-1.5 py-0.5 rounded-full">{s.grade}</span>}
                      {s.comments && <span className="text-xs text-slate-500 flex-1 truncate">{s.comments}</span>}
                    </div>
                  ))}
                  {lvl.total_reviewers === 0 && <p className="text-xs text-slate-500">Not reviewed</p>}
                </div>
              ))}
            </div>
          );
